...
class PlagiarismScanner {
  constructor(copyleaksClient, webhookBaseUrl, productEndpoint = 'scans', baseUrl = 'https://api.copyleaks.com') {
  ...
  async submitTextScan(scanId, text, options = {}) {
    ...
    try {
      ...
      if (error.response?.status === 401) {
        ...
        throw new Error('Access forbidden. Your API key may not have permission for plagiarism scanning.');
        ...
        throw new Error('Could not connect to Copyleaks API. Please check your internet connection.');
      } else {
        throw new Error(`Plagiarism scan failed: ${error.response?.data?.message || error.response?.data?.error || error.message}`);
      }
    }
  }
  ...
  async exportResults(scanId, resultIds) {
    ...
    try {
      ...
      if (error.response?.status === 404) {
        ...
        throw new Error(`Export failed: ${error.response?.data?.message || 'Invalid export parameters'}`);
      } else {
        throw new Error(`Export failed: ${error.response?.data?.message || error.response?.data?.error || error.message}`);
      }
    }
  }
